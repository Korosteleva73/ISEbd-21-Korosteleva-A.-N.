...
namespace CarFactoryBusinessLogic.BusinessLogics
{
    static class SaveToWord
    {
        public static void CreateDoc(WordInfo info)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var car in info.Cars)
                {
                    docBody.AppendChild(CreateParagraph(new WordParagraph
                    {
                        Texts = new List<(string, WordTextProperties)> { ($"{car.CarName}: ", new WordTextProperties { Size = "24", Bold = true }), (car.Price.ToString(), new WordTextProperties { Size = "24" }) },
                        TextProperties = new WordTextProperties
                        {
                            Size = "24",
       ...
       paragraphProperties)
        ...
        public static void CreateWarehouseDoc(WarehouseWordInfo info)
        {
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(info.FileName, WordprocessingDocumentType.Document))
            {
                MainDocumentPart mainPart = wordDocument.AddMainDocumentPart();
                ...
                foreach (var warehouse in info.Warehouses)
                {
                    ...
                    cellFIO.Append(new TableCellProperties(
                        new TableCellWidth() { Type = TableWidthUnitValues.Dxa, Width = "4000" }));
                    cellFIO.Append(new Paragraph(new Run(new Text(warehouse.WarehouseBoss))));
