...
namespace CarFactoryBusinessLogic.BusinessLogics
{
    static class SaveToExcel
    {
        public static void CreateDoc(ExcelInfo info)
        {
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                foreach (var cd in info.CarDetails)
                {
                    ...
                    rowIndex++;
                    foreach (var material in cd.Details)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
        ...
        public static void CreateWarehouseDoc(WarehouseExcelInfo info)
        {
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                // Создаем книгу (в ней хранятся листы)
                ...
                uint rowIndex = 2;
                foreach (ReportWarehouseDetailsViewModel wd in info.WarehouseDetails)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
