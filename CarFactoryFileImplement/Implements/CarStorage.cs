...
namespace CarFactoryFileImplement.Implements
{
    public class CarStorage : ICarStorage
    {
        ...
        public CarViewModel GetElement(CarBindingModel model)
        {
            ...
            var car = source.Cars
            .FirstOrDefault(rec => rec.CarName == model.CarName || rec.Id
           == model.Id);
            return car != null ? CreateModel(car) : null;
        }
        public void Insert(CarBindingModel model)
        {
            int maxId = source.Cars.Count > 0 ? source.Details.Max(rec => rec.Id) : 0;
            var element = new Car
            {
                Id = maxId + 1,
                CarDetails = new
        ...
        private CarViewModel CreateModel(Car car)
        {
            return new CarViewModel
            {
                ...
                CarDetails = car.CarDetails
                 .ToDictionary(recCD => recCD.Key, recCD =>
                 (source.Details.FirstOrDefault(recD => recD.Id ==
                recCD.Key)?.DetailName, recCD.Value))
            };
        }
    }
}
