    ...
    public partial class FormCars : Form
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                generic.Invoke(this, new object[] { logic.Read(null), dataGridViewCars });
            }
            catch (Exception ex)
            {
        ...
        private void ButtonUpd_Click(object sender, EventArgs e)
        {
            ...
            {
                var form = Container.Resolve<FormCar>();
                form.Id = Convert.ToInt32(dataGridViewCars.SelectedRows[0].Cells[0].Value);
                if (form.ShowDialog() == DialogResult.OK)
                {
                    LoadData();
                }
        ...
        private void ButtonDel_Click(object sender, EventArgs e)
        {
            if (dataGridViewCars.SelectedRows.Count == 1)
            {
               ...
               MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    int id = Convert.ToInt32(dataGridViewCars.SelectedRows[0].Cells[0].Value);
                    try
                    {
